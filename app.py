...
# ==============================================================================
# 0. CONFIGURACIÓN GLOBAL (V64 ESTABLE)
# ==============================================================================
DB_NAME = 'sgsst_v64_stable_restore.db' # Nombre nuevo para limpieza total
...
# BASE DE DATOS INTERNA PARA IRL (ESTABLE Y PROBADA)
IRL_DATA_DB = {
...
def init_erp_db():
    ...
    # Matriz (Tabla simple, sin inserción masiva conflictiva)
...
def generar_pdf_irl(rut_trabajador, area, h_ini, h_fin, estatus):
    ...
    try:
        ...
        # BUSQUEDA EN DICCIONARIO INTERNO (ESTABLE)
        ...
        if data_cargo.get('riesgos'):
            h = [Paragraph("RIESGO", s_h), Paragraph("CONSECUENCIA", s_h), Paragraph("MEDIDA", s_h), Paragraph("MÉTODO", s_h)]
            d = [h]
        ...
        elements.append(Paragraph("<b>3. CARACTERISTICAS DEL LUGAR</b>", s_title))
        ...
        elements.append(Spacer(1, 15))
        elements.append(Paragraph("<b>4. SUSTANCIAS Y EMERGENCIAS</b>", s_title))
        elements.append(Paragraph(f"Sustancia: {data_cargo['sustancia']}. Conocer Plan de Emergencia y Vías de Evacuación.", ParagraphStyle('N', fontSize=8)))
...
if menu == "👥 Nómina & Personal":
    ...
    if sel:
        ...
        with st.form("irl"):
            ...
            st.markdown("### Contenidos"); st.write(f"Lugar: {base['lugar']}"); st.write(f"Riesgos: {len(base['riesgos'])} detectados")
            st_user = st.selectbox("Estatus", ["Nuevo", "Reinduccion"]); sub = st.form_submit_button("Generar PDF")
