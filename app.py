...
# ==============================================================================
# 1. CAPA DE DATOS (SQL RELACIONAL) - V14 (PDF Fix Final)
# ==============================================================================
def init_erp_db():
    conn = sqlite3.connect('sgsst_v14_pdf_fix.db')
    c = conn.cursor()
...
def generar_pdf_asistencia_rggd02(id_cap):
    ...
    try:
        ...
        # FIX: Safe String Conversion for Date
        now_str = datetime.now().strftime('%d/%m/%Y')
        
        control_data = [["REGISTRO DE CAPACITACIÓN"], ["CODIGO: RG-GD-02"], ["VERSION: 01"], [f"FECHA: {now_str}"], ["PAGINA: 1"]]
        ...
        # --- FIX: Safe String Handling (Prevents 'int' object is not iterable / NoneType error) ---
        def safe_str(val):
...
# --- 2. GESTIÓN NÓMINA ---
    ...
    st.title("Plan de Capacitación y Entrenamiento"); st.markdown("**Formato Oficial: RG-GD-02**"); tab_prog, tab_firma, tab_hist = st.tabs(["📅 Crear Nueva", "✍️ Asignar/Enviar a Móvil", "🗂️ Historial y PDF"]); conn = sqlite3.connect('sgsst_v14_pdf_fix.db')
    with tab_prog:
    ...
    with tab_firma:
        ...
        if not caps_activas.empty:
            ...
            def enviar_asistentes_callback(id_cap, df_trab):
                c_cb = sqlite3.connect('sgsst_v14_pdf_fix.db'); cursor_cb = c_cb.cursor(); selection = st.session_state.selector_asistentes
                if selection:
    ...
    st.title("Generador de IRL Automático"); conn = sqlite3.connect('sgsst_v14_pdf_fix.db'); users = pd.read_sql("SELECT nombre, cargo FROM personal", conn); sel = st.selectbox("Trabajador:", users['nombre']); st.write(f"Generando documento para cargo: **{users[users['nombre']==sel]['cargo'].values[0]}**"); st.button("Generar IRL (Simulación)"); conn.close()

elif menu == "⚠️ Matriz IPER":
    st.title("Matriz de Riesgos"); conn = sqlite3.connect('sgsst_v14_pdf_fix.db'); df_iper = pd.read_sql("SELECT * FROM matriz_iper", conn); st.dataframe(df_iper); conn.close()

elif menu == "🔐 Gestión Usuarios" and st.session_state['user_role'] == "ADMINISTRADOR":
    st.title("Administración de Usuarios del Sistema"); conn = sqlite3.connect('sgsst_v14_pdf_fix.db')
