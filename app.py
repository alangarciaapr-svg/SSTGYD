...
# --- 2. GESTIÓN NÓMINA (TUS TRABAJADORES REALES) ---
    ...
    # BOTÓN PARA DESCARGAR PLANTILLA (SOLICITADO EN TURNO ANTERIOR)
    col_plantilla, col_upload = st.columns([1, 2])
    with col_plantilla:
        st.info("¿Necesitas el formato?")
        def generar_plantilla_excel():
            output = io.BytesIO()
            data = {'NOMBRE': [], 'RUT': [], 'CARGO': [], 'LUGAR DE TRABAJO': []}
            df_template = pd.DataFrame(data)
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                pd.DataFrame(["PLANTILLA DE CARGA - NO BORRAR ESTAS 2 PRIMERAS FILAS"]).to_excel(writer, startrow=0, startcol=0, index=False, header=False)
        ...
        st.download_button("📥 Bajar Plantilla", data=plantilla_data, file_name="plantilla_carga_nomina.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
