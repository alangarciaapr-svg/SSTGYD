...
# ==============================================================================
# 1. CAPA DE DATOS (SQL RELACIONAL) - V31 (Logo Aspect Ratio Fix)
# ==============================================================================
def init_erp_db():
    conn = sqlite3.connect('sgsst_v31_final_logo.db') 
    c = conn.cursor()
...
def generar_pdf_riohs(id_reg):
    ...
    try:
        ...
        t_det = Table(data_det, colWidths=[150, 250]); 
        ...
        elements.append(Paragraph("__________________________<br/>FIRMA TRABAJADOR", style_center))
        elements.append(Spacer(1, 20))
        elements.append(Paragraph("Este documento constituye un registro válido del Sistema de Gestión de Seguridad y Salud en el Trabajo.", style_center))
        doc.build(elements); buffer.seek(0); return buffer
    except: return None
...
# --- 2. GESTIÓN NÓMINA ---
    ...
    st.title("Plan de Capacitación y Entrenamiento"); st.markdown("**Formato Oficial: RG-GD-02**"); tab_prog, tab_firma, tab_hist = st.tabs(["📅 Crear Nueva", "✍️ Asignar/Enviar a Móvil", "🗂️ Historial y PDF"]); conn = sqlite3.connect('sgsst_v31_final_logo.db')
    with tab_prog:
    ...
    with tab_firma:
        ...
        if not caps_activas.empty:
            ...
            def enviar_asistentes_callback(id_cap, df_trab):
                c_cb = sqlite3.connect('sgsst_v31_final_logo.db'); cursor_cb = c_cb.cursor(); selection = st.session_state.selector_asistentes
                if selection:
    ...
    with tab_hist:
        ...
        if not historial.empty:
            ...
            conn_sig = sqlite3.connect('sgsst_v31_final_logo.db')
    ...
    st.title("Generador de IRL Automático"); conn = sqlite3.connect('sgsst_v31_final_logo.db'); users = pd.read_sql("SELECT nombre, cargo FROM personal", conn); sel = st.selectbox("Trabajador:", users['nombre']); st.write(f"Generando documento para cargo: **{users[users['nombre']==sel]['cargo'].values[0]}**"); st.button("Generar IRL (Simulación)"); conn.close()

elif menu == "⚠️ Matriz IPER":
    st.title("Matriz de Riesgos"); conn = sqlite3.connect('sgsst_v31_final_logo.db'); df_iper = pd.read_sql("SELECT * FROM matriz_iper", conn); st.dataframe(df_iper); conn.close()

elif menu == "🔐 Gestión Usuarios" and st.session_state['user_role'] == "ADMINISTRADOR":
    st.title("Administración de Usuarios del Sistema"); conn = sqlite3.connect('sgsst_v31_final_logo.db')
