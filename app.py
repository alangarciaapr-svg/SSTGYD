...
DB_NAME = 'sgsst_v185_final_irl_fix.db'  # mantengo tu nombre para compatibilidad
...
# ==============================================================================
# 1.1 HELPERS ROBUSTOS
# ==============================================================================
def clean_str(val):
    """Normaliza strings para DB (evita 'nan', 'NaT', '', None)."""
    if val is None:
        return None
    s = str(val).strip()
...
def safe_int0(val):
    return safe_int(val, default=0)

def safe_date_from_any(val):
...
# ==============================================================================
# 2. CAPA DE DATOS (SQL) - MEJORADA (WAL + timeout)
# ==============================================================================
def get_conn():
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS conducta_personal (
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS inventario_epp (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        producto TEXT,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS contratistas (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        rut_empresa TEXT,
        razon_social TEXT,
        estado_documental TEXT,
        fecha_vencimiento_f30 DATE
    )''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS periodos_ds67 (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nombre_periodo TEXT,
        fecha_inicio DATE,
        fecha_fin DATE
    ...
    check_and_add_column(c, "incidentes", "dias_perdidos", "INTEGER")  # clave para DS67

    # Seed de usuario admin + inventario solo si usuarios vacíos
    ...
    # Seed de matriz_iper (FIX: ya NO depende de usuarios==0)
    c.execute("SELECT count(*) FROM matriz_iper")
    if c.fetchone()[0] == 0:
        c.execute("""INSERT INTO matriz_iper
            (proceso, tipo_proceso, puesto_trabajo, tarea, es_rutinaria, peligro_factor, riesgo_asociado, tipo_riesgo,
             probabilidad, consecuencia, vep, nivel_riesgo, medida_control, genero_obs, familia_riesgo, lugar_especifico,
             n_hombres, n_mujeres, n_disidencias, requisito_legal, probabilidad_residual, consecuencia_residual, vep_residual, nivel_riesgo_residual)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            ("Cosecha", "Operativo", "Operador", "Tala", "SI", "Pendiente", "Volcamiento", "Seguridad",
             2, 4, 8, "IMPORTANTE", "Cabina ROPS", "Sin Obs", "Seguridad", "Bosque",
             5, 0, 0, "DS 594 Art 11", 1, 4, 4, "MODERADO")
        )
...
def get_alertas():
    ...
    try:
        ...
        for _, t in trabs.iterrows():
            if t.get('vigencia_examen_medico'):
                try:
                    fv = pd.to_datetime(t['vigencia_examen_medico'], errors='coerce')
                    if pd.isna(fv):
                        continue
                    fv = fv.date()
                    if fv < hoy:
                        alertas.append(f"🔴 {t['nombre']}: Examen Vencido")
                    elif fv < hoy + timedelta(days=30):
                        alertas.append(f"🟡 {t['nombre']}: Examen vence pronto")
                except:
                    pass
    except:
        pass
    ...
    try:
        ...
        for _, s in stock.iterrows():
            alertas.append(f"📦 Stock Bajo: {s['producto']}")
    except:
        pass
...
def get_incidentes_mes():
    ...
    try:
        ...
        res = pd.read_sql(f"SELECT count(*) as c FROM incidentes WHERE strftime('%m', fecha)='{mes}'", conn).iloc[0, 0]
    except:
        res = 0
    conn.close()
    return res
...
def enviar_correo_riohs(destinatario, nombre_trabajador, pdf_bytes, nombre_archivo):
    ...
    if EMAIL_EMISOR == "tu_correo@empresa.com":
        time.sleep(0.5)
        return True, "Simulación: Correo enviado (Configurar SMTP real)"
...
# ==============================================================================
# 3. MOTOR DOCUMENTAL (PDF) - MEJORADO (logo URL)
# ==============================================================================
class DocumentosLegalesPDF:
    def __init__(self, titulo_doc, codigo_doc):
        self.buffer = io.BytesIO()
        self.doc = SimpleDocTemplate(self.buffer, pagesize=letter, topMargin=30, leftMargin=30, rightMargin=30)
        self.elements = []
        self.styles = getSampleStyleSheet()
        self.titulo = titulo_doc
        self.codigo = codigo_doc
        self.logo_url = "https://www.maderasgyd.cl/wp-content/uploads/2024/02/logo-maderas-gd-1.png"

    def _header(self):
        # FIX: RLImage no soporta URL directo -> descargamos y usamos BytesIO
        try:
            with urlopen(self.logo_url) as r:
                img_bytes = r.read()
            logo = RLImage(io.BytesIO(img_bytes), width=120, height=50)
        except Exception:
    ...
    def _signature_block(self, firma_b64):
        ...
        if firma_b64:
            if firma_b64 == "PENDIENTE_DIGITAL":
                sig_img = Paragraph(
                    "ENVÍO DIGITAL MASIVO - PENDIENTE DE ACUSE",
                    ParagraphStyle('P', fontSize=8, alignment=TA_CENTER, textColor=colors.red)
                )
            else:
                try:
                    sig_img = RLImage(io.BytesIO(base64.b64decode(firma_b64)), width=250, height=100)
                except:
                    pass
    ...
    def generar_reporte_ds67(self, data_ds67):
        ...
        t_res.setStyle(TableStyle([
            ('GRID', (0, 0), (-1, -1), 0.5, colors.black),
            ('BACKGROUND', (0, 0), (0, 4), HexColor(COLOR_PRIMARY)),
        ...
        for d in data_ds67['detalle']:
            det.append([f"{d['mes']}/{d['anio']}", str(d['masa_imponible']), str(d['dias_perdidos']), str(d['invalideces_muertes'])])
    ...
    def generar_epp(self, data):
        ...
        d_info = [[f"NOMBRE: {data['nombre']}", f"RUT: {data['rut']}"],
        ...
        for i in items:
            t_data.append([str(i.get('cant', '1')), i.get('prod', 'EPP'), i.get('talla', 'U')])
    ...
    def generar_riohs(self, data):
        ...
        if "Digital" in str(data.get('tipo_entrega', '')):
        ...
        if data.get('firma_b64'):
            try:
                sig_img = RLImage(io.BytesIO(base64.b64decode(data['firma_b64'])), width=200, height=80)
            except:
                pass
        ...
        if data.get('firma_difusor'):
            try:
                sig_dif = RLImage(io.BytesIO(base64.b64decode(data['firma_difusor'])), width=180, height=70)
            except:
                pass
    ...
    def generar_irl(self, data, riesgos):
        ...
        for r in riesgos:
            r_data.append([
                Paragraph(str(r[0]), ParagraphStyle('s', fontSize=8)),
                Paragraph(str(r[1]), ParagraphStyle('s', fontSize=8)),
                Paragraph(str(r[2]), ParagraphStyle('s', fontSize=8))
            ])
        ...
        self.elements.append(Paragraph("Recibí información (DS44).", self.styles['Normal']))
        self.elements.append(Spacer(1, 30))
    ...
    def generar_asistencia_capacitacion(self, data, asis):
        ...
        for a in asis:
            ...
            if a.get('firma_b64'):
                try:
                    f_img = RLImage(io.BytesIO(base64.b64decode(a['firma_b64'])), width=100, height=30)
                except:
                    pass
            a_data.append([a.get('nombre_trabajador', a.get('nombre', '')), a.get('trabajador_rut', a.get('rut', '')), f_img])
            row_h.append(40)
    ...
    def generar_diat(self, data):
        ...
        self.elements.append(Paragraph(f"AFECTADO: {data['nombre']}", self.styles['Heading3']))
        self.elements.append(Spacer(1, 20))
        self.elements.append(Paragraph(data.get('descripcion', ''), self.styles['Normal']))
...
# ==============================================================================
# 4. UI/FRONTEND - CSS + KIOSK MODE ROBUSTO
# ==============================================================================
# --- MODO KIOSCO (robusto) ---
query_params = st.query_params
mobile_sign = str(query_params.get("mobile_sign", "false")).lower() == "true"
...
if mobile_sign:
    ...
    cap_id_int = None
    try:
        cap_id_int = int(str(cap_id_mobile))
    except:
        cap_id_int = None
    ...
    if cap_id_int:
        conn = get_conn()
        try:
            cap_data = pd.read_sql("SELECT tema, fecha FROM capacitaciones WHERE id=?", conn, params=(cap_id_int,))
                ...
                with st.form("mobile_sign_form"):
                    ...
                    if st.form_submit_button("ENVIAR FIRMA"):
                        if rut_input and canvas_mobile.image_data is not None:
                            check = pd.read_sql(
                                "SELECT id FROM asistencia_capacitacion WHERE capacitacion_id=? AND trabajador_rut=?",
                                conn,
                                params=(cap_id_int, rut_input)
                            )
                            if not check.empty:
                                ...
                                conn.execute(
                                    "UPDATE asistencia_capacitacion SET estado='FIRMADO', firma_b64=? WHERE capacitacion_id=? AND trabajador_rut=?",
                                    (img_str, cap_id_int, rut_input)
                                )
                                conn.commit()
                                st.success("✅ Firma registrada. Gracias.")
                            else:
                                st.error("❌ RUT no inscrito.")
                        else:
                            st.warning("⚠️ Faltan datos.")
            else:
                st.error("Capacitación no encontrada.")
        except Exception as e:
            st.error(f"Error de conexión con DB Móvil: {e}")
        conn.close()
    else:
        st.error("Capacitación no válida.")
    st.stop()
...
if not st.session_state['logged_in']:
    ...
    with c2:
        st.markdown(f"""<div class="logo-box"><img src="{LOGO_URL}" style="width: 100%; max-width: 220px;"></div>
        <h4 style='text-align: center; color: #444; margin-bottom: 20px; font-weight:600;'>INICIAR SESIÓN</h4>""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    try:
        ...
        st.warning(f"No se pudo abrir la BD para respaldo: {e}")
...
# ==============================================================================
# 8. UTIL PARA GUARDAR EDITORES SIMPLES (Plan/Ext/Contratistas)
# ==============================================================================
def replace_table_from_df(conn, table_name, df, columns_order):
    """
    Estrategia simple y estable: DELETE + INSERT (como inventario).
    Funciona bien cuando no hay llaves foráneas críticas.
    """
    c = conn.cursor()
    c.execute(f"DELETE FROM {table_name}")
    placeholders = ",".join(["?"] * len(columns_order))
    cols = ",".join(columns_order)
    ins = f"INSERT INTO {table_name} ({cols}) VALUES ({placeholders})"
...
if menu == "📊 Dashboard":
    st.markdown(
        f"""<div style="background-color: {COLOR_SECONDARY}; padding: 20px; border-radius: 10px; margin-bottom: 20px; color: white;">
        <h2 style="margin:0; color:white;">Bienvenido, {st.session_state['user'].capitalize()}</h2>
        <p style="margin:0;">Sistema de Gestión SST | {date.today().strftime('%d-%m-%Y')}</p>
        </div>""",
        unsafe_allow_html=True
    )
    ...
    try:
        acc = pd.read_sql("SELECT count(*) as c FROM incidentes", conn).iloc[0, 0]
        cap = pd.read_sql("SELECT count(*) as c FROM capacitaciones", conn).iloc[0, 0]
        trabs = pd.read_sql("SELECT count(*) as c FROM personal WHERE estado='ACTIVO'", conn).iloc[0, 0]
        alertas = get_alertas()
    except:
        acc = 0
        cap = 0
    ...
    def card(t, v, i):
        return f"<div class='card-kpi'><h3>{t}</h3><h1>{i} {v}</h1></div>"
    ...
    with c1:
        ...
        if alertas:
            with st.container(height=300):
                for a in alertas:
                    st.markdown(f"<div class='alert-box alert-high'>⚠️ {a}</div>", unsafe_allow_html=True)
        else:
            st.success("Todo OK")
    ...
    with t2:
        ...
        if not periods.empty:
            ...
            with c2:
                ...
                try:
                    query_inc = f"""
                        SELECT COALESCE(SUM(dias_perdidos),0) as dp
                        FROM incidentes
                        WHERE strftime('%m', fecha) = '{mes_sel:02d}'
                          AND strftime('%Y', fecha) = '{anio_sel}'
                    """
                    dias_res = pd.read_sql(query_inc, conn).iloc[0, 0]
                    dias_auto = int(dias_res) if dias_res is not None else 0
                except:
                    dias_auto = 0
            ...
            if st.button("Guardar Registro Mensual"):
                conn.execute("""INSERT INTO detalle_mensual_ds67 (periodo_id, mes, anio, masa_imponible, dias_perdidos, invalideces_muertes)
                                VALUES (?,?,?,?,?,?)""",
    ...
    with t3:
        ...
        if not periods.empty:
            ...
            if not data_raw.empty:
                ...
                factor_inv = total_inv * 2500  # DS67
                ...
                k2.metric("Total Días Perdidos", int(total_dias))
                ...
                if st.button("Generar Reporte Oficial PDF"):
                    pdf = DocumentosLegalesPDF("REPORTE TASA DE SINIESTRALIDAD DS67", "INF-DS67-01").generar_reporte_ds67({
                        'periodo': sel_rep,
                        'masa': round(avg_masa, 1),
                        'dias': int(total_dias),
                        'inv': int(total_inv),
                        'tasa': tasa_siniestralidad,
                        'cot': cotizacion,
                        'detalle': data_raw.to_dict('records')
                    })
                    st.download_button("Descargar Informe", pdf.getvalue(), "Informe_DS67.pdf", mime="application/pdf")
            else:
...
# ==============================================================================
    ...
    with tab_ver:
        ...
        filtro_puesto = cf3.multiselect("Puesto Trabajo", df_matriz['PUESTO'].unique().tolist() if not df_matriz.empty else [])

        df_view = df_matriz.copy()
        if filtro_nivel:
            df_view = df_view[df_view['NIVEL_INI'].isin(filtro_nivel)]
        if filtro_fam:
            df_view = df_view[df_view['FAMILIA'].isin(filtro_fam)]
        if filtro_puesto:
            df_view = df_view[df_view['PUESTO'].isin(filtro_puesto)]
            ...
            with g1:
                if not df_view.empty:
                    fig_heat = px.density_heatmap(
                        df_view, x="P_INI", y="C_INI",
                        title="Mapa de Calor (Probabilidad vs Consecuencia)",
                        nbinsx=4, nbinsy=4, color_continuous_scale="Reds"
                    )
                    st.plotly_chart(fig_heat, use_container_width=True)
            with g2:
                if not df_view.empty:
                    total_h = int(df_view['H'].fillna(0).sum())
                    total_m = int(df_view['M'].fillna(0).sum())
                    total_d = int(df_view['D'].fillna(0).sum())
        ...
        edited_df = st.data_editor(
            ...
            key="matriz_ed",
            column_config={
                "id": st.column_config.NumberColumn("ID", disabled=True),  # FIX: no editable
        ...
        if st.button("💾 Guardar y Recalcular Matriz"):
            ...
            for _, r in edited_df.iterrows():
                rid = safe_int(r.get('id'), default=None)
                if rid is None:
                    continue

                pi = safe_int(r.get('P_INI'), default=1)
                ci = safe_int(r.get('C_INI'), default=1)
                vi = pi * ci
                ni = calcular_nivel_riesgo(vi)

                pr = safe_int(r.get('P_RES'), default=1)
                cr = safe_int(r.get('C_RES'), default=1)
                vr = pr * cr
                nr = calcular_nivel_riesgo(vr)

                c.execute("""
                    UPDATE matriz_iper SET
                ...
                """, (
                    pi, ci, vi, ni,
                    pr, cr, vr, nr,
                    clean_str(r.get('MEDIDA CONTROL')),
                    clean_str(r.get('FAMILIA')),
                    clean_str(r.get('COD ISP')),
                    clean_str(r.get('JERARQUIA')),
                    clean_str(r.get('LEGAL')),
                    clean_str(r.get('GEMA')),
                    clean_str(r.get('LUGAR')),
                    safe_int0(r.get('H')),
                    safe_int0(r.get('M')),
                    safe_int0(r.get('D')),
                    rid
    ...
    with tab_carga:
        ...
        with pd.ExcelWriter(b2, engine='openpyxl') as w:
            pd.DataFrame(plantilla).to_excel(w, index=False)
        ...
        if up:
            try:
                ...
                if 'P_Inicial' in df_up.columns and 'C_Inicial' in df_up.columns:
                    df_up['Estado'] = df_up.apply(
                        lambda x: "⚠️ Error P/C" if x['P_Inicial'] not in [1, 2, 4] or x['C_Inicial'] not in [1, 2, 4] else "✅ OK",
                        axis=1
                    )
                else:
                    st.error("El archivo no tiene las columnas P_Inicial o C_Inicial. Descargue la plantilla.")
                    st.stop()
                ...
                if st.button("3️⃣ Procesar Carga Definitiva"):
                    c = conn.cursor()
                    count_ok = 0
                    for _, r in edited_up.iterrows():
                        if r.get('Estado') == "✅ OK":
                            pi = safe_int(r.get('P_Inicial'), default=1)
                            ci = safe_int(r.get('C_Inicial'), default=1)
                            vi = pi * ci
                            ni = calcular_nivel_riesgo(vi)

                            pr = safe_int(r.get('P_Residual'), default=1)
                            cr = safe_int(r.get('C_Residual'), default=1)
                            vr = pr * cr
                            nr = calcular_nivel_riesgo(vr)
                            ...
                            c.execute("""INSERT INTO matriz_iper
                                (proceso, puesto_trabajo, lugar_especifico, familia_riesgo, factor_gema, peligro_factor, riesgo_asociado,
                                 ...
                                 probabilidad_residual, consecuencia_residual, vep_residual, nivel_riesgo_residual)
                                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
                                clean_str(r.get('Proceso')),
                                clean_str(r.get('Puesto')),
                                clean_str(r.get('Lugar')),
                                clean_str(r.get('Familia')),
                                clean_str(r.get('GEMA')),
                                clean_str(r.get('Peligro')),
                                clean_str(r.get('Riesgo')),
                                safe_int0(r.get('Hombres')),
                                safe_int0(r.get('Mujeres')),
                                safe_int0(r.get('Diversidad')),
                                pi, ci, vi, ni,
                                clean_str(r.get('Medida')),
                                clean_str(r.get('Jerarquia')),
                                clean_str(r.get('Legal')),
                                pr, cr, vr, nr
                            ))
            ...
            if st.form_submit_button("Guardar Evaluación Maestra"):
                ...
                conn.execute("""INSERT INTO matriz_iper
                    (proceso, puesto_trabajo, lugar_especifico, familia_riesgo, codigo_riesgo, factor_gema, peligro_factor, riesgo_asociado,
                     ...
                     probabilidad_residual, consecuencia_residual, vep_residual, nivel_riesgo_residual)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                             (pro, pue, lug, fam, cod_clean, gema, pel, rie, nh, nm, nd,
                              pi, ci, vi, ni, med, jc, leg, pr, cr, vr, nr))
                conn.commit()
                st.success("Riesgo Maestro Guardado")
                st.rerun()

    conn.close()

# ==============================================================================
# 12. GESTION PERSONAS (FIX date_input opcional)
# ==============================================================================
elif menu == "👥 Gestión Personas":
    st.markdown("<div class='main-header'>Gestión de Capital Humano</div>", unsafe_allow_html=True)
    conn = get_conn()
    try:
        total = pd.read_sql("SELECT count(*) as c FROM personal", conn).iloc[0, 0]
        activos = pd.read_sql("SELECT count(*) as c FROM personal WHERE estado='ACTIVO'", conn).iloc[0, 0]
    except:
        total = 0
        activos = 0
    ...
    k2.metric("Activos", activos)
    k3.metric("Bajas", total - activos)

    tab_list, tab_carga, tab_new, tab_dig, tab_del = st.tabs(["📋 Nómina", "📂 Carga Masiva", "➕ Nuevo", "🗂️ Carpeta", "❌ Eliminar"])

    with tab_list:
        df_p = pd.read_sql("""SELECT rut, rut as rut_old, nombre, cargo, centro_costo, email,
                              fecha_contrato, vigencia_examen_medico, contacto_emergencia, fono_emergencia, estado
                              FROM personal""", conn)
        ...
        edited = st.data_editor(
            df_p, key="pers_ed", use_container_width=True, num_rows="dynamic",
            column_config={
                "rut_old": None,
        ...
        if st.button("💾 Guardar Cambios"):
            ...
            for _, r in edited.iterrows():
                fec = r.get('fecha_contrato')
                f_ex = r.get('vigencia_examen_medico')

                if pd.isna(fec) or str(fec) == 'NaT':
                    fec = date.today()
                else:
                    try:
                        fec = pd.to_datetime(fec).date()
                    except:
                        fec = date.today()

                if pd.isna(f_ex) or str(f_ex) == 'NaT':
                    f_ex = None
                else:
                    try:
                        f_ex = pd.to_datetime(f_ex).date()
                    except:
                        f_ex = None

                rut_old = clean_str(r.get('rut_old'))
                rut_new = clean_str(r.get('rut'))
                rut_key = rut_old if rut_old else rut_new

                if rut_new != rut_old and rut_old:
                    c.execute("""UPDATE personal SET rut=?, nombre=?, cargo=?, centro_costo=?, email=?, estado=?, fecha_contrato=?, vigencia_examen_medico=?,
                                 contacto_emergencia=?, fono_emergencia=? WHERE rut=?""",
                              (rut_new, clean_str(r.get('nombre')), clean_str(r.get('cargo')), clean_str(r.get('centro_costo')),
                               clean_str(r.get('email')), clean_str(r.get('estado')), fec, f_ex,
                               clean_str(r.get('contacto_emergencia')), clean_str(r.get('fono_emergencia')),
                               rut_key))
                    c.execute("UPDATE asistencia_capacitacion SET trabajador_rut=? WHERE trabajador_rut=?", (rut_new, rut_key))
                    c.execute("UPDATE registro_epp SET rut_trabajador=? WHERE rut_trabajador=?", (rut_new, rut_key))
                    c.execute("UPDATE registro_riohs SET rut_trabajador=? WHERE rut_trabajador=?", (rut_new, rut_key))
                else:
                    c.execute("""UPDATE personal SET nombre=?, cargo=?, centro_costo=?, email=?, estado=?, fecha_contrato=?, vigencia_examen_medico=?,
                                 contacto_emergencia=?, fono_emergencia=? WHERE rut=?""",
                              (clean_str(r.get('nombre')), clean_str(r.get('cargo')), clean_str(r.get('centro_costo')),
                               clean_str(r.get('email')), clean_str(r.get('estado')), fec, f_ex,
                               clean_str(r.get('contacto_emergencia')), clean_str(r.get('fono_emergencia')),
                               rut_key))
            conn.commit()
    ...
    with tab_carga:
        template_data = {
            'RUT': ['11.222.333-4'], 'NOMBRE': ['Juan Pérez'], 'CARGO': ['OPERADOR DE MAQUINARIA'],
        ...
        if up:
            if st.button("🚀 Procesar"):
                try:
                    ...
                    for _, r in df.iterrows():
                        rut = clean_str(r.get('RUT'))
                        if not rut or len(rut) <= 3:
                            continue

                        f = safe_date_from_any(r.get('FECHA DE CONTRATO')) or date.today()
                        f_ex = safe_date_from_any(r.get('VIGENCIA_EXAMEN'))
                        ...
                        c.execute("""INSERT OR REPLACE INTO personal
                            ...
                            VALUES (?,?,?,?,?,?,?,?,?,?)""",
                                  (rut, clean_str(r.get('NOMBRE')), clean_str(r.get('CARGO')), clean_str(r.get('CENTRO_COSTO')),
                                   clean_str(r.get('EMAIL')), f, 'ACTIVO', f_ex,
                                   clean_str(r.get('CONTACTO_EMERGENCIA')), clean_str(r.get('TELEFONO_EMERGENCIA'))))
                    conn.commit()
                    st.success("Cargado")
                    time.sleep(1)
                    st.rerun()
    ...
    with tab_new:
        with st.form("new_p"):
            ...
            ca = c1.selectbox("Cargo", LISTA_CARGOS)
            em = c2.text_input("Email")

            c3, c4 = st.columns(2)
            use_ex = c3.checkbox("Ingresar vencimiento examen", value=False)
            f_ex = c3.date_input("Vencimiento Examen", value=date.today()) if use_ex else None  # FIX: evita date_input(None)

            c_emer = c4.text_input("Contacto Emergencia")
            f_emer = c3.text_input("Teléfono Emergencia")
            obs = c4.text_input("Alergias/Obs Médica")

            if st.form_submit_button("Registrar"):
                try:
                    conn.execute("""INSERT INTO personal
                        (rut, nombre, cargo, email, fecha_contrato, estado, vigencia_examen_medico, contacto_emergencia, fono_emergencia, obs_medica)
                        VALUES (?,?,?,?,?,?,?,?,?,?)""",
                                 (clean_str(r), clean_str(n), clean_str(ca), clean_str(em), date.today(), 'ACTIVO',
                                  f_ex, clean_str(c_emer), clean_str(f_emer), clean_str(obs)))
                    conn.commit()
                    st.success("Creado")
                except Exception as e:
                    st.error(f"Error (RUT duplicado?): {e}")
    ...
    with tab_dig:
        ...
        if not df_all.empty:
            ...
            datos_p = pd.read_sql("SELECT contacto_emergencia, fono_emergencia, obs_medica, vigencia_examen_medico FROM personal WHERE rut=?",
                                  conn, params=(rut_sel,)).iloc[0]
            st.info(f"🚑 Emergencia: {datos_p.get('contacto_emergencia')} - {datos_p.get('fono_emergencia')} | ⚕️ Obs: {datos_p.get('obs_medica')}")

            st.subheader("🚦 Estado de Habilitación")
            odi = pd.read_sql("SELECT count(*) as c FROM asistencia_capacitacion WHERE trabajador_rut=?", conn, params=(rut_sel,)).iloc[0, 0] > 0
            riohs = pd.read_sql("SELECT count(*) as c FROM registro_riohs WHERE rut_trabajador=?", conn, params=(rut_sel,)).iloc[0, 0] > 0
            epp = pd.read_sql("SELECT count(*) as c FROM registro_epp WHERE rut_trabajador=?", conn, params=(rut_sel,)).iloc[0, 0] > 0

            f_examen_raw = datos_p.get('vigencia_examen_medico')
            examen_ok = False
            if f_examen_raw:
                try:
                    f_ex_dt = pd.to_datetime(f_examen_raw, errors='coerce')
                    if not pd.isna(f_ex_dt) and f_ex_dt.date() >= date.today():
                        examen_ok = True
                except:
                    pass
            ...
            hist = pd.read_sql("SELECT fecha, tipo, descripcion, gravedad FROM conducta_personal WHERE rut_trabajador=?",
                               conn, params=(rut_sel,))
            if not hist.empty:
            ...
            with st.expander("➕ Agregar Registro de Conducta"):
                with st.form("conducta_form"):
                    f_c = st.date_input("Fecha Evento")
                    t_c = st.selectbox("Tipo", ["Amonestación Verbal", "Amonestación Escrita", "Felicitación", "Incidente"])
                    g_c = st.selectbox("Gravedad", ["Leve", "Grave", "Gravísima", "Positiva"])
                    d_c = st.text_area("Descripción")
                    if st.form_submit_button("Guardar Registro"):
                        conn.execute("INSERT INTO conducta_personal (rut_trabajador, fecha, tipo, descripcion, gravedad) VALUES (?,?,?,?,?)",
                                     (rut_sel, f_c, t_c, d_c, g_c))
                        conn.commit()
...
# ==============================================================================
# 13. GESTOR DOCUMENTAL (IRL + RIOHS) - SE MANTIENE
# ==============================================================================
    ...
    with t_riohs:
        if df_p.empty:
            ...
            with tab_ind:
                ...
                with st.form("riohs_form"):
                    ...
                    with c3:
                        ...
                        sig_worker = st_canvas(stroke_width=2, stroke_color="black", background_color="#eeeeee", height=150, width=400, key="sig_w_riohs_v186")
                    with c4:
                        st.write("Firma Difusor:")
                        sig_diffuser = st_canvas(stroke_width=2, stroke_color="black", background_color="#eeeeee", height=150, width=400, key="sig_d_riohs_v186")
                    ...
                    if st.form_submit_button("Registrar Entrega RIOHS"):
                        if sig_worker.image_data is not None and sig_diffuser.image_data is not None and nom_dif:
                            img_w = process_signature_bg(sig_worker.image_data)
                            b_w = io.BytesIO()
                            img_w.save(b_w, format='PNG')
                            str_w = base64.b64encode(b_w.getvalue()).decode()

                            img_d = process_signature_bg(sig_diffuser.image_data)
                            b_d = io.BytesIO()
                            img_d.save(b_d, format='PNG')
                            str_d = base64.b64encode(b_d.getvalue()).decode()

                            tipo_db = "Digital" if "Digital" in tipo_ent else "Físico"

                            pdf = DocumentosLegalesPDF(
                            ...
                            conn.execute("""INSERT INTO registro_riohs
                                (fecha_entrega, rut_trabajador, nombre_trabajador, tipo_entrega, firma_b64, nombre_difusor, firma_difusor_b64, email_copia, estado_envio)
                                VALUES (?,?,?,?,?,?,?,?,?)""",
                                         (date.today(), rut_w, nom_w, tipo_db, str_w, nom_dif, str_d, email_w, estado_envio))
                            conn.commit()

                            st.download_button("📥 Descargar Acta", pdf_bytes, f"RIOHS_{rut_w}.pdf", mime="application/pdf")
                            st.success("Entrega registrada.")
                        else:
                            st.warning("⚠️ Faltan firmas o nombre del difusor.")
            ...
            with tab_mass:
                ...
                sig_mass = st_canvas(stroke_width=2, stroke_color="black", background_color="#eeeeee", height=150, width=400, key="sig_mass_v186")

                if st.button("🚀 INICIAR CAMPAÑA", type="primary"):
                    if difusor_mass and sig_mass.image_data is not None:
                        img_d = process_signature_bg(sig_mass.image_data)
                        b_d = io.BytesIO()
                        img_d.save(b_d, format='PNG')
                        str_d = base64.b64encode(b_d.getvalue()).decode()

                        targets = pd.read_sql("""SELECT rut, nombre, cargo, email FROM personal
                                                 WHERE estado='ACTIVO' AND email IS NOT NULL AND email != ''""", conn)
                        ...
                        for i, t in targets.iterrows():
                            pdf = DocumentosLegalesPDF(
                                ...
                                'email': t['email'],
                                'nombre_difusor': difusor_mass,
                                'firma_difusor': str_d
                            })

                            exito, _ = enviar_correo_riohs(t['email'], t['nombre'], pdf.getvalue(), f"RIOHS_{t['rut']}.pdf")
                            ...
                            count += 1
                            prog_bar.progress((i + 1) / max(1, len(targets)))
...
# ==============================================================================
# 14. LOGISTICA EPP (se mantiene tu flujo)
# ==============================================================================
    ...
    with t1:
        ...
        if df_workers.empty:
            ...
            if 'epp_step' not in st.session_state:
                st.session_state.epp_step = 1

            if st.session_state.epp_step == 1:
                if st.button("📦 Cargar Kit Básico (Nuevo Ingreso)"):
                    kit = [
                        {"prod": "CASCO DE SEGURIDAD", "cant": 1, "talla": "U", "precio": 12000},
                        {"prod": "LENTES DE SEGURIDAD", "cant": 1, "talla": "U", "precio": 3000},
                        {"prod": "GUANTE CABRITILLA", "cant": 1, "talla": "U", "precio": 4500},
                        {"prod": "CHALECO REFLECTANTE", "cant": 1, "talla": "L", "precio": 3500}
                    ]
                ...
                if inv_df.empty:
                    ...
                    with c1:
                        ...
                        last_del = pd.read_sql(
                            "SELECT fecha_entrega FROM registro_epp WHERE rut_trabajador=? AND lista_productos LIKE ? ORDER BY id DESC LIMIT 1",
                            conn, params=(rut_w, f"%{prod_sel}%")
                        )
                        if not last_del.empty:
                            try:
                                f_last = pd.to_datetime(last_del.iloc[0, 0], errors='coerce')
                                if not pd.isna(f_last):
                                    f_last = f_last.date()
                                    dias = (date.today() - f_last).days
                                    if dias < 30:
                                        st.warning(f"⚠️ {prod_sel} entregado hace solo {dias} días ({f_last})")
                                    else:
                                        st.caption(f"✅ Última entrega: {f_last} (Hace {dias} días)")
                            except:
                                pass
                    ...
                    with c3:
                        talla_sel = st.selectbox("Talla", ["U", "S", "M", "L", "XL", "XXL", "38", "39", "40", "41", "42", "43", "44", "45"], index=0)
                    with c4:
                        st.write("")
                        if st.button("➕ Agregar"):
                            row = inv_df[inv_df['producto'] == prod_sel].iloc[0]
                            if int(row['stock_actual']) >= int(cant_sel):
                                st.session_state.epp_cart.append({"prod": prod_sel, "cant": int(cant_sel), "talla": talla_sel, "precio": int(row['precio'] or 0)})
                            else:
                                st.error("Stock insuficiente.")

                if st.session_state.epp_cart:
                    st.write("---")
                    st.markdown("##### Resumen de Entrega y Costos")
                    cart_df = pd.DataFrame(st.session_state.epp_cart)
                    cart_df['Subtotal'] = cart_df['cant'] * cart_df['precio']
                    total_cost = int(cart_df['Subtotal'].sum())
                    st.table(cart_df)
                    st.metric("Costo Total Estimado", f"${total_cost:,.0f}")
                    ...
                    if st.button("✅ CONFIRMAR ENTREGA"):
                        if firm_canvas.image_data is not None:
                            ...
                            conn.execute("""INSERT INTO registro_epp
                                (fecha_entrega, rut_trabajador, nombre_trabajador, cargo, lista_productos, firma_b64)
                                VALUES (?,?,?,?,?,?)""", (date.today(), rut_w, nom_w, cargo_w, str(st.session_state.epp_cart), img_str))

                            for item in st.session_state.epp_cart:
                                conn.execute("UPDATE inventario_epp SET stock_actual = stock_actual - ? WHERE producto=?",
                                             (int(item['cant']), item['prod']))
                            conn.commit()

                            pdf = DocumentosLegalesPDF("REGISTRO ENTREGA ELEMENTOS DE PROTECCION PERSONAL", "RG-SSTGD-01").generar_epp({
                                'nombre': nom_w, 'rut': rut_w, 'cargo': cargo_w, 'fecha': date.today().strftime("%d-%m-%Y"),
                                'lista': str(st.session_state.epp_cart), 'firma_b64': img_str
                            })
                            st.session_state.pdf_buffer = pdf.getvalue()
                            st.session_state.epp_step = 2
                            st.rerun()
                        else:
                ...
                st.download_button("📥 DESCARGAR ACTA RG-SSTGD-01", st.session_state.pdf_buffer, file_name=f"EPP_{rut_w}.pdf", mime="application/pdf")
                st.write("")
    ...
    with t2:
        ...
        edited_inv = st.data_editor(
            current_inv, num_rows="dynamic", use_container_width=True, key="inv_editor",
            column_config={"precio": st.column_config.NumberColumn("Precio Unitario ($)", format="$%d")}
        )
        if st.button("💾 Guardar Cambios Inventario"):
            ...
            for _, r in edited_inv.iterrows():
                ...
                if isinstance(p, float) and pd.isna(p):
                    p = 0
                conn.execute("""INSERT INTO inventario_epp (producto, stock_actual, stock_minimo, ubicacion, precio)
                                VALUES (?,?,?,?,?)""",
                             (clean_str(r.get('producto')), safe_int0(r.get('stock_actual')),
                              safe_int0(r.get('stock_minimo')), clean_str(r.get('ubicacion')), int(p)))
            conn.commit()
...
# ==============================================================================
    ...
    with t1:
        with st.form("cap"):
            ...
            tp = st.selectbox("Tipo", ["Inducción", "Charla"])
            lug = st.text_input("Lugar")
            dur = st.number_input("Horas", 1)
            rel = st.text_input("Relator")

            df_p = pd.read_sql("SELECT rut, nombre FROM personal", conn)
            asis = st.multiselect("Asistentes", df_p['rut'] + " | " + df_p['nombre'] if not df_p.empty else [])

            if st.form_submit_button("Guardar"):
                c = conn.cursor()
                c.execute("INSERT INTO capacitaciones (fecha, tema, tipo_actividad, responsable_rut, lugar, duracion) VALUES (?,?,?,?,?,?)",
                          (date.today(), t, tp, rel, lug, dur))
                cid = c.lastrowid
                for a in asis:
                    c.execute("INSERT INTO asistencia_capacitacion (capacitacion_id, trabajador_rut, nombre_trabajador, estado) VALUES (?,?,?,?)",
                              (cid, a.split(" | ")[0], a.split(" | ")[1], "PENDIENTE"))
                conn.commit()
                st.success("OK")
    ...
    with t2:
        ...
        if not caps.empty:
            ...
            try:
                ...
                default_url = "https://tu-app.streamlit.app"
            ...
            st.info("No hay capacitaciones creadas aún.")

    with t3:
        caps_all = pd.read_sql("SELECT * FROM capacitaciones", conn)
        st.dataframe(caps_all, use_container_width=True)
        if not caps_all.empty:
            sel_pdf = st.selectbox("PDF:", caps_all['id'].astype(str) + " - " + caps_all['tema'])
            if st.button("Generar Lista"):
                ...
                pdf = DocumentosLegalesPDF("CAPACITACION", "RG-GD-02").generar_asistencia_capacitacion(
                    {'tema': c_data['tema'], 'tipo': c_data['tipo_actividad'], 'resp': c_data['responsable_rut'],
                     'fecha': c_data['fecha'], 'lugar': c_data.get('lugar'), 'duracion': c_data.get('duracion')},
                    a_data
                )
                st.download_button("PDF", pdf.getvalue(), "Lista.pdf", mime="application/pdf")

    conn.close()

# ==============================================================================
# 16. INCIDENTES & DIAT (FIX: dias_perdidos para DS67)
# ==============================================================================
    ...
    with st.form("inc"):
        ...
        a = st.text_input("Afectado")
        dp = st.number_input("Días perdidos", min_value=0, value=0)  # FIX DS67

        if st.form_submit_button("Guardar"):
            conn.execute("INSERT INTO incidentes (fecha, descripcion, nombre_afectado, dias_perdidos) VALUES (?,?,?,?)", (f, d, a, int(dp)))
            conn.commit()
            st.success("OK")

    st.divider()
    st.dataframe(pd.read_sql("SELECT id, fecha, nombre_afectado, dias_perdidos, descripcion FROM incidentes ORDER BY id DESC", conn),
                 use_container_width=True)

    if st.button("Generar DIAT"):
        pdf = DocumentosLegalesPDF("DIAT", "LEGAL").generar_diat({
            'nombre': 'Ejemplo',
            'rut': '1-9',
        ...
        st.download_button("DIAT", pdf.getvalue(), "DIAT.pdf", mime="application/pdf")

    conn.close()

# ==============================================================================
# 17. PLAN ANUAL (FIX: ahora guarda)
# ==============================================================================
elif menu == "📅 Plan Anual":
    st.title("Plan Anual")
    conn = get_conn()
    df_plan = pd.read_sql("SELECT id, actividad, responsable, fecha_programada, estado, fecha_ejecucion FROM programa_anual", conn)
    df_plan['fecha_programada'] = pd.to_datetime(df_plan['fecha_programada'], errors='coerce')
    df_plan['fecha_ejecucion'] = pd.to_datetime(df_plan['fecha_ejecucion'], errors='coerce')

    edited_plan = st.data_editor(df_plan, key="plan", num_rows="dynamic", use_container_width=True)
    if st.button("💾 Guardar Plan Anual"):
        # guardado estable: DELETE + INSERT
        tmp = edited_plan.copy()
        if 'id' in tmp.columns:
            tmp = tmp.drop(columns=['id'])
        replace_table_from_df(conn, "programa_anual", tmp, ["actividad", "responsable", "fecha_programada", "estado", "fecha_ejecucion"])
        st.success("Plan anual guardado.")
        time.sleep(0.7)
        st.rerun()
    conn.close()

# ==============================================================================
# 18. EXTINTORES (FIX: ahora guarda)
# ==============================================================================
elif menu == "🧯 Extintores":
    st.title("Extintores")
    conn = get_conn()
    df_ext = pd.read_sql("SELECT id, codigo, tipo, capacidad, ubicacion, fecha_vencimiento, estado_inspeccion FROM extintores", conn)
    df_ext['fecha_vencimiento'] = pd.to_datetime(df_ext['fecha_vencimiento'], errors='coerce')

    edited_ext = st.data_editor(df_ext, key="ext", num_rows="dynamic", use_container_width=True)
    if st.button("💾 Guardar Extintores"):
        tmp = edited_ext.copy()
        if 'id' in tmp.columns:
            tmp = tmp.drop(columns=['id'])
        replace_table_from_df(conn, "extintores", tmp, ["codigo", "tipo", "capacidad", "ubicacion", "fecha_vencimiento", "estado_inspeccion"])
        st.success("Extintores guardados.")
        time.sleep(0.7)
        st.rerun()
    conn.close()

# ==============================================================================
# 19. CONTRATISTAS (FIX: ahora guarda)
# ==============================================================================
elif menu == "🏗️ Contratistas":
    st.title("Contratistas")
    conn = get_conn()
    df_con = pd.read_sql("SELECT id, rut_empresa, razon_social, estado_documental, fecha_vencimiento_f30 FROM contratistas", conn)
    df_con['fecha_vencimiento_f30'] = pd.to_datetime(df_con['fecha_vencimiento_f30'], errors='coerce')

    edited_con = st.data_editor(df_con, key="con", num_rows="dynamic", use_container_width=True)
    if st.button("💾 Guardar Contratistas"):
        tmp = edited_con.copy()
        if 'id' in tmp.columns:
            tmp = tmp.drop(columns=['id'])
        replace_table_from_df(conn, "contratistas", tmp, ["rut_empresa", "razon_social", "estado_documental", "fecha_vencimiento_f30"])
        st.success("Contratistas guardados.")
        time.sleep(0.7)
        st.rerun()
    conn.close()

# ==============================================================================
# 20. GESTIÓN USUARIOS (se mantiene)
# ==============================================================================
    ...
    try:
        total_u = pd.read_sql("SELECT count(*) as c FROM usuarios", conn).iloc[0, 0]
        admins = pd.read_sql("SELECT count(*) as c FROM usuarios WHERE rol='ADMINISTRADOR'", conn).iloc[0, 0]
    except:
        total_u = 0
        admins = 0
