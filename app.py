...
# ==============================================================================
# 1. CAPA DE DATOS (SQL RELACIONAL) - V34 (RIOHS Final Style)
# ==============================================================================
def init_erp_db():
    conn = sqlite3.connect('sgsst_v34_final_style.db') 
    c = conn.cursor()
...
def generar_pdf_asistencia_rggd02(id_cap):
    ...
    try:
        ...
        if os.path.exists(LOGO_FILE):
            logo_img = get_scaled_logo(LOGO_FILE, 85, 55)
...
def generar_pdf_riohs(id_reg):
    ...
    try:
        ...
        d_fec = Paragraph(f"<b>FECHA RECEPCIÓN:</b> {fecha}", style_center)
        d_fmt = Paragraph(f"<b>FORMATO:</b> {tipo}", style_center)
        
        # Build dynamic table with email if exists
        rows_data = [[d_nom, d_rut], [d_fec, d_fmt]]
        if correo and len(correo) > 3:
            rows_data.append([Paragraph(f"<b>CORREO:</b> {correo}", style_center), ""])

        t_personal = Table(rows_data, colWidths=[250, 250])
        t_personal.setStyle(TableStyle([('GRID', (0,0), (-1,-1), 1, colors.black), ('BACKGROUND', (0,0), (-1,-1), colors.whitesmoke), ('VALIGN', (0,0), (-1,-1), 'MIDDLE')]))
        elements.append(t_personal); elements.append(Spacer(1, 30))

        txt = f"""Yo, <b>{nom_t}</b>, RUT <b>{rut_t}</b>, declaro haber recibido gratuitamente una copia del Reglamento Interno de Orden, Higiene y Seguridad de la empresa.<br/><br/>Me comprometo a leerlo, estudiarlo y cumplir fielmente las disposiciones, normas y procedimientos en él contenidos, así como las instrucciones que la empresa imparta en materias de prevención de riesgos."""
        style_just = ParagraphStyle('Just', parent=styles['Normal'], alignment=TA_JUSTIFY, fontSize=11, leading=14)
        elements.append(Paragraph(txt, style_just)); elements.append(Spacer(1, 60))
...
# --- 2. GESTIÓN NÓMINA ---
    ...
    st.title("Plan de Capacitación y Entrenamiento"); st.markdown("**Formato Oficial: RG-GD-02**"); tab_prog, tab_firma, tab_hist = st.tabs(["📅 Crear Nueva", "✍️ Asignar/Enviar a Móvil", "🗂️ Historial y PDF"]); conn = sqlite3.connect('sgsst_v34_final_style.db')
    with tab_prog:
    ...
    with tab_firma:
        ...
        if not caps_activas.empty:
            ...
            def enviar_asistentes_callback(id_cap, df_trab):
                c_cb = sqlite3.connect('sgsst_v34_final_style.db'); cursor_cb = c_cb.cursor(); selection = st.session_state.selector_asistentes
                if selection:
    ...
    with tab_hist:
        ...
        if not historial.empty:
            ...
            conn_sig = sqlite3.connect('sgsst_v34_final_style.db')
    ...
    with st.form("form_riohs"):
        ...
        # Canvas fuera del form para evitar refresh
    ...
    st.title("Generador de IRL Automático"); conn = sqlite3.connect('sgsst_v34_final_style.db'); users = pd.read_sql("SELECT nombre, cargo FROM personal", conn); sel = st.selectbox("Trabajador:", users['nombre']); st.write(f"Generando documento para cargo: **{users[users['nombre']==sel]['cargo'].values[0]}**"); st.button("Generar IRL (Simulación)"); conn.close()

elif menu == "⚠️ Matriz IPER":
    st.title("Matriz de Riesgos"); conn = sqlite3.connect('sgsst_v34_final_style.db'); df_iper = pd.read_sql("SELECT * FROM matriz_iper", conn); st.dataframe(df_iper); conn.close()

elif menu == "🔐 Gestión Usuarios" and st.session_state['user_role'] == "ADMINISTRADOR":
    st.title("Administración de Usuarios del Sistema"); conn = sqlite3.connect('sgsst_v34_final_style.db')
